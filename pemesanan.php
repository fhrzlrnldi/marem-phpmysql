...
class pemesanan extends db
{
    ...
    {
        $data = $this->db->prepare("SELECT * FROM pemesanan JOIN pegawai ON pegawai.id_pegawai=pemesanan.id_pegawai JOIN calon_konsumen ON calon_konsumen.id_calon_konsumen=pemesanan.id_calon_konsumen JOIN penawaran ON penawaran.id_penawaran=pemesanan.id_penawaran");
        try
        {
            $data->execute();
    ...
    {
        $data = $this->db->prepare("SELECT * FROM pegawai");
        try
        {
            $data->execute();
            $result = $data->fetchAll();
    ...
    {
        $data = $this->db->prepare("UPDATE pemesanan SET id_pegawai = ?, id_calon_konsumen = ?, id_penawaran =  ?, tgl_pemesanan = ?, status_pemesanan = ?, alamat_pengiriman = ?, total_harga = ? WHERE id_pemesanan = ?");

        $data->bindParam(1, $id_pegawai);
        $data->bindParam(2, $id_calon_konsumen);
        $data->bindParam(3, $id_penawaran);
        $data->bindParam(4, $tgl_pemesanan);
        $data->bindParam(5, $status_pemesanan);
        $data->bindParam(6, $alamat_pengiriman);
        $data->bindParam(7, $total_harga);
        $data->bindParam(8, $id_pemesanan);
        try
        {
            $result = $data->execute();
        }
...
if(isset($_POST['submit'])){
    ...
    $data->store($id_pemesanan, $id_pegawai, $id_calon_konsumen, $id_penawaran, $tgl_pemesanan, $status_pemesanan, $alamat_pengiriman, $total_harga);
...
if(isset($_POST['update'])){
    ...
    $alamat_pengiriman = $_POST ['alamat_pengiriman'];
    $total_harga = $_POST ['total_harga'];
    $data->update($id_pemesanan, $id_pegawai, $id_calon_konsumen, $id_penawaran, $tgl_pemesanan, $status_pemesanan, $alamat_pengiriman, $total_harga);
    $_SESSION["sukses"] = 'Data Berhasil Disimpan';
    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item"><a href="pemesanan.php">Data Pemesanan</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
...
<div class="d-flex justify-content-end">
    <a class="btn btn-primary mb-3" data-bs-toggle="modal" data-bs-target="#insert">Insert Data Pemesanan</a>
</div>
<div class="modal fade" id="insert" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">Insert Data Pemesanan</h5>
        ...
        <div class="modal-body">
            ...
            <div class="row d-flex align-items-center justify-content-between my-3">
                ...
                <div class="col-lg-7">
                    <input required class="form-control" type="text" name="id_pemesanan" placeholder="ID Pemesanan">
                </div>
            </div>
            <div class="row d-flex align-items-center justify-content-between my-3">
                <div class="col-5">Pegawai</div>
                <div class="col-lg-7">
                    <select name="id_pegawai" required class="form-control">
                        <?php
                        foreach ($data->showpegawai() as $row)
                        {
            ...
            <div class="row d-flex align-items-center justify-content-between my-3">
                <div class="col-5">Tanggal Pemesanan</div>
                <div class="col-lg-7">
                    <input required class="form-control" type="datetime-local" name="tgl_pemesanan" placeholder="Tanggal Pemesanan">
                </div>
            </div>
            <div class="row d-flex align-items-center justify-content-between my-3">
                <div class="col-5">Status Pemesanan</div>
                <div class="col-lg-7">
                    <select name="status_pemesanan" required class="form-control">
                        <option value="01">Selesai</option>
                        <option value="00">Belum Bayar</option>
                    </select>
                </div>
            </div>
            <div class="row d-flex align-items-center justify-content-between my-3">
                <div class="col-5">Alamat Pengiriman</div>
                <div class="col-lg-7">
                    <input required class="form-control" type="text" name="alamat_pengiriman" placeholder="Alamat Pengiriman">
                </div>
            </div>
            <div class="row d-flex align-items-center justify-content-between my-3">
                <div class="col-5">Total Harga</div>
                <div class="col-lg-7">
                    <input required class="form-control" type="number" name="total_harga" placeholder="Total Harga">
                </div>
            </div>
    ...
    <thead>
                ...
                <?php foreach ($data->show() as $value): ?>
                                        <tr>
                                            ...
                                            <td><?php if($value['status_pemesanan'] == "01"){echo "Selesai";} else {echo "Belum Dibayar";} ?></td>
                                            <td><?= $value['alamat_pengiriman'] ?></td>
                                            <td><?= $value['total_harga'] ?></td>
                                            <td>
                                                <div class="d-flex justify-content-center">
                                                    <button class="btn btn-sm btn-warning mx-1" data-bs-toggle="modal" data-bs-target="#update<?php echo $value['id_pemesanan']; ?>">Update</button>
                                                    <button class="btn btn-sm btn-danger mx-1" data-bs-toggle="modal" data-bs-target="#delete<?php echo $value['id_pemesanan']; ?>">delete</button>
                                                </div>
                                            </td>
                                        </tr>
                                        <div class="modal fade" id="update<?php echo $value['id_pemesanan']; ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                        <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="exampleModalLabel">Update Data Pemesanan</h5>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                            </div>
                                            <div class="modal-body">
                                                <form method="POST">
                                                <input required class="form-control" type="hidden" name="id_pemesanan" value="<?php echo $value['id_pemesanan'] ?>">

                                                <div class="row d-flex align-items-center justify-content-between my-3">
                                                    <div class="col-5">Pegawai</div>
                                                    <div class="col-lg-7">
                                                        <select name="id_pegawai" required class="form-control">
                                                            <option value="<?php echo $value['id_pegawai'] ?>" selected><?php
                                                                {echo $value['nama_pegawai'];}?>
                                                            </option>
                                                            <?php
                                                            foreach ($data->showpegawai() as $row)
                                                            {
                                                ...
                                                <div class="row d-flex align-items-center justify-content-between my-3">
                                                    ...
                                                    <div class="col-lg-7">
                                                        <select name="id_calon_konsumen" required class="form-control">
                                                            <option value="<?php echo $value['id_calon_konsumen'] ?>" selected><?php
                                                                {echo $value['nama_calon_konsumen'];}?>
                                                            </option>
                                                            <?php
                                                            foreach ($data->showcalonkonsumen() as $row)
                                                            {
                                                ...
                                                <div class="row d-flex align-items-center justify-content-between my-3">
                                                    ...
                                                    <div class="col-lg-7">
                                                        <select name="id_penawaran" required class="form-control">
                                                            <option value="<?php echo $value['id_penawaran'] ?>" selected><?php
                                                                {echo $value['tgl_penawaran'];}?>
                                                            </option>
                                                            <?php
                                                            foreach ($data->showpenawaran() as $row)
                                                            {
                                                ...
                                                <div class="row d-flex align-items-center justify-content-between my-3">
                                                    <div class="col-5">Tanggal Pemesanan</div>
                                                    <div class="col-lg-7">
                                                        <input required class="form-control" type="datetime-local" name="tgl_pemesanan" value="<?php echo $value['tgl_pemesanan'] ?>">
                                                    </div>
                                                </div>
                                                <div class="row d-flex align-items-center justify-content-between my-3">
                                                    <div class="col-5">Status Pemesanan</div>
                                                ...
                                                <div class="row d-flex align-items-center justify-content-between my-3">
                                                    <div class="col-5">Alamat Pengiriman</div>
                                                    <div class="col-lg-7">
                                                        <input required class="form-control" type="text" name="alamat_pengiriman" value="<?php echo $value['alamat_pengiriman'] ?>">
                                                    </div>
                                                </div>
                                                <div class="row d-flex align-items-center justify-content-between my-3">
                                                    <div class="col-5">Total Harga</div>
                                                    <div class="col-lg-7">
                                                        <input required class="form-control" type="number" name="total_harga" value="<?php echo $value['total_harga'] ?>">
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="row d-flex justify-content-center">
                                        ...
                                        <div class="modal fade" id="delete<?php echo $value['id_pemesanan']; ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                            <div class="modal-dialog">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="exampleModalLabel">Delete Data Pemesanan</h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body">
                                                    <form method="POST">
